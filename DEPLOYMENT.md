...
6. После проверки `/health` и ручного `/digest` установите
   `SHADOW_MODE=false`.
7. `HH_POLL_INTERVAL_SECONDS=14400` задаёт общий цикл мониторинга раз в четыре
   часа. Первый сбор после запуска выполняется сразу, затем используется этот
   интервал.
8. После пополнения OpenAI API новый деплой перезапускает AI-оценку и один раз
   пересчитывает не более 20 свежих вакансий, ранее оценённых резервным
   алгоритмом. `SCORING_THRESHOLD=70` применяется к сильным совпадениям;
   `SCORING_FALLBACK_THRESHOLD=50` задаёт уровень для предварительной вечерней
   подборки. Если не достигнут и он, бот отправит до пяти лучших вариантов для
   ручной проверки вместо сообщения о пустом результате.

## LinkedIn через Gmail

1. Создайте Gmail OAuth client со scope
   `https://www.googleapis.com/auth/gmail.readonly`.
2. Настройте LinkedIn Job Alerts.
3. Создайте Gmail-метку `LinkedIn-Jobs` и направляйте в неё уведомления.
   Название можно изменить через `GMAIL_LABEL`.
4. Добавьте `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN` и
   включите `GMAIL_ENABLED=true`.
5. HH-рассылки через Gmail и Mail.ru намеренно не используются: письма не
   содержат полного описания вакансии и не позволяют выполнить достоверную
   автоматическую оценку. HeadHunter подключается только через официальный API,
   если доступ к нему будет одобрен.
...
## Habr Career и международные вакансии

Эти источники работают без пользовательского OAuth и без доступа к аккаунтам:

- `HABR_JOBS_ENABLED=true` — RSS Habr Career используется как триггер, затем
  бот читает полную публичную карточку удалённой вакансии;
- `INTERNATIONAL_JOBS_ENABLED=true` — публичные JSON-ленты Himalayas и Jobicy;
- `PUBLIC_JOBS_POLL_INTERVAL_SECONDS=3600` — повторный сетевой запрос не чаще
  одного раза в час;
- `PUBLIC_JOBS_MAX_RESULTS=50` — максимум карточек из каждого источника за цикл.

До AI-оценки проходят только целевые руководящие роли и удалённые вакансии.
Для страновых ограничений допускаются Worldwide/Russia, а также позиции с
релокацией, визовой поддержкой или contractor/B2B-форматом. Повторы между
источниками объединяются, а более полное описание обновляет уже найденную
карточку и запускает повторную оценку.
